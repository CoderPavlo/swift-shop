...
import { Alert, AlertTitle, Box, Button, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, FormControl, Grid, InputLabel, MenuItem, Select, Skeleton, Stack, TextField, Typography } from '@mui/material'
...
export default function GoodsPage() {
    ...
    const [filter, setFilter] = React.useState<IGoodFilterForShop>({ categoryId: -1, searchQuery: '', order: true, });
    ...
    const { data: goods, error: goodsError, isLoading: goodsLoading} = goodAPI.useFetchGoodsByShopQuery(filter, {refetchOnMountOrArgChange: true});
    ...
    const changeFilter = (type: 'category' | 'order' | 'search', value: number | string | boolean) => {
        if (type === 'category') setFilter({ ...filter, categoryId: value as number });
        else if (type === 'order') setFilter({ ...filter, order: value as boolean });
        else {
            if ((new Date).getTime() - startTime.getTime() > 500) {
                clearTimeout(typingTimer);
                typingTimer = setTimeout(() => setFilter({ ...filter, searchQuery: value as string }), 500);
            }
            return;
        }
    }
    ...
    return (
        <>
            ...
            {categoriesLoading ?
                ...
                : categoriesError ?
                    ...
                    <Stack padding={1} display='flex' justifyContent="space-between" flexDirection="row" alignItems='center'>
                        <TextField placeholder='Пошук...' size='small' sx={{ width: { md: '35%' } }} onChange={(e) => changeFilter('search', e.target.value)} />
            ...
            <GoodDialog open={goodProcessing.action === 'add' || goodProcessing.action === 'edit'} handleClose={handleClose} type={goodProcessing.action === 'edit' ? 'edit' : 'add'} 
                data={goodProcessing.action === 'edit' && goodForChange? {id: goodForChange.id,
                    ...
                    tags: goodForChange.tags,} : undefined} loading={goodForChangeLoading} error={Boolean(goodForChangeError)}
            />
            <Dialog
