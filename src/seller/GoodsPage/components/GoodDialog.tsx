...
import { IGoodFormData, ITag } from '../../../models/IGood';
import { Formik } from 'formik';
import * as Yup from 'yup';
import { LoadingButton } from '@mui/lab';
import { useAppDispatch } from '../../../store/hooks';
import { updateTokens } from '../../../store/reducers/authSlice';
...
export default function GoodDialog({ open, handleClose, type, data = initialValues }: IGoodDialogProps) {
        ...
        >
            ...
            <Formik
                ...
                onSubmit={async (values) => {
                    let accessToken = await dispatch(updateTokens());
                    ...
                    if (newTags.length > 0)
                        await addTags({ body: newTags, token: accessToken }).unwrap()
                            .then((payload: ITag[]) => {
            ...
            >
                {({ errors, handleBlur, handleChange, handleSubmit, setFieldValue, touched, values }) => (
                    <form noValidate onSubmit={handleSubmit}>
                                            ...
                                            <>
                                                <Autocomplete
                                                    onChange={(event, newValue) => {
                                                        if (newValue) setFieldValue('categories', [newValue, ...values.categories]);
                                                    }}
                                                    options={categories}
                        ...
                        <DialogActions>
                            <LoadingButton
                                loading={loadingTagsAdd || loadingGood}
